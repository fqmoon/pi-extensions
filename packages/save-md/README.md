...
A Pi extension that saves the latest assistant response, optionally with recent conversation context, as Markdown.
...
```bash
pi install npm:@fqmoon/pi-save-md
```

During development, install the repository or load `packages/save-md/index.ts` directly from Pi.
